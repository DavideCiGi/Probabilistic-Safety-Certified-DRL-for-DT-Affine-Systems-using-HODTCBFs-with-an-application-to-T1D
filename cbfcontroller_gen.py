...
class DT_CBFs:
    def __init__(self, dt, nominal_model_params, CBF1_params, CBF2_params, CHO_max, GP_max_size, episode_length,
        ...
        gamma_1 = [sp.Symbol(f'\gamma_{{{i + 1}, 1}}', real=True) for i in range(r + 1)]
        gamma_2 = [sp.Symbol(f'\gamma_{{{i + 1}, 2}}', real=True) for i in range(r + 1)]
        ...
        psi_actual = psi.copy()   # CHECK IF .COPY IS NECESSARY HERE
        phi_actual = phi.copy()
        ...
        psi = psi.subs(dict(zip(G_temp, G)))  # CHECK
        phi = phi.subs(dict(zip(G_temp, G)))
        ...
        psi = psi.subs(nominal_model_params).subs(CBF1_params)
        phi = phi.subs(nominal_model_params).subs(CBF2_params)

        psi_true = psi_true.subs(true_model_params).subs(CBF1_params)  # of course we shouldn't have access to the true
        # parameters. This is just a way to double check if what we're doing is ok or not. You can even eliminate it
        phi_true = phi_true.subs(true_model_params).subs(CBF2_params)
        ...
        psi = psi.subs({CHO[0]: 0})
        phi0 = phi.subs({CHO[0]: 0})  # function we'll use later for GPs
        phi = phi.subs({CHO[0]: CHO_max})

        psi_true = psi_true.subs({CHO[0]: 0})
        phi_true = phi_true.subs({CHO[0]: CHO_max})

        psi_functions = sp.lambdify(
            (G[0], X[0], I[0], D_2[0], D_1[0], ID[0]), psi[:, 0], modules='numpy')
        phi0_functions = sp.lambdify(
            (G[0], X[0], I[0], D_2[0], D_1[0], ID[0]), phi0[:, 0], modules='numpy')
        psi_initial_functions = sp.lambdify(
            (G[0], X[0], I[0], D_2[0], D_1[0], ID[0]), psi[:r, 0], modules='numpy')
        phi_initial_functions = sp.lambdify(
            (G[0], X[0], I[0], D_2[0], D_1[0], ID[0]), phi[:r, 0], modules='numpy')
        psi_r_function = sp.lambdify(
            (G[0], X[0], I[0], D_2[0], D_1[0], ID[0]), psi[r, 0], modules='numpy')
        phi_r_function = sp.lambdify(
            (G[0], X[0], I[0], D_2[0], D_1[0], ID[0]), phi[r, 0], modules='numpy')
        psi_true_r_function = sp.lambdify(
            (G[0], X[0], I[0], D_2[0], D_1[0], ID[0]), psi_true[r, 0], modules='numpy')
        phi_true_r_function = sp.lambdify(
            (G[0], X[0], I[0], D_2[0], D_1[0], ID[0]), phi_true[r, 0], modules='numpy')

        self.psi_functions = psi_functions
        self.phi0_functions = phi0_functions
        self.psi_initial_functions = psi_initial_functions
        self.phi_initial_functions = phi_initial_functions
        ...
        self.GP_memory = GPBuffer(GP_max_size, state_dim, action_dim, r, episode_length)
        self.X_ht = HistoryTracker(r, 1, state_dim + action_dim)

        self.BG_max_value = BG_max_value
        self.BG_min_value = BG_min_value

    def initial_check_CBF(self, x0):
        psi_eval = self.psi_initial_functions(x0[0], x0[1], x0[2], x0[3], x0[4], 0.0)
        phi_eval = self.phi_initial_functions(x0[0], x0[1], x0[2], x0[3], x0[4], 0.0)
        ...
        for i in range(self.r):
            if psi_eval[i] >= 0:
                k += 1
        if k == self.r:
            print("The first CBF can be initially satisfied in average,")
        else:
            print("The first CBF cannot be initially satisfied in average,")
        k = 0
        for i in range(self.r):
            if phi_eval[i] >= 0:
                k += 1
        if k == self.r:
            print("the second CBF can be initially satisfied in average.")
        else:
            print("the second CBF cannot be initially satisfied in average.")
    ...
    def GP_collect_data(self, eating):
        if not eating:
            previous_state_action = self.X_ht.history[-1]
            psi_error = self.psi_actual.copy()
            phi_error = self.phi_actual.copy()
            psi_estimate = (self.psi_functions(*previous_state_action)).copy()  # maybe .copy() a bit redundant here
            phi_estimate = (self.phi0_functions(*previous_state_action)).copy()
            for i in range(1, self.r + 1):
                for k in range(i+1):
                    psi_error[i] = (psi_error[i]).subs({self.G_temp[k]: self.X_ht.history[self.r-k, 0]})
                    phi_error[i] = (phi_error[i]).subs({self.G_temp[k]: self.X_ht.history[self.r-k, 0]})
                psi_error[i] -= psi_estimate[i, 0]
                phi_error[i] -= phi_estimate[i, 0]
            self.GP_memory.store(previous_state_action, psi_error[1:], phi_error[1:])
...
class CBF_with_GPs_SOCP:
    def __init__(self, k_delta, K_eps_1, K_eps_2, max_action, min_action, gamma_1_dict, gamma_2_dict):
        ...
        r = len(gamma_1)
        psi_beta_list = [beta(j, r, gamma_1) for j in range(1, r + 1)]
        self.psi_betas = np.array(psi_beta_list).reshape((r, 1))
        phi_beta_list = [beta(j, r, gamma_2) for j in range(1, r + 1)]
        self.phi_betas = np.array(phi_beta_list).reshape((r, 1))
    ...
    def solve(self, a_11, a_21, a_12, a_22, psi_m_r, psi_m_1, phi_m_r, phi_m_1, psi_Lr_bar, phi_Lr_bar,
        ...
        b_3 = np.squeeze(self.k_delta * (psi_Lr_bar @ self.psi_betas + psi_L1_bar * (u_RL + u_bar)))
        b_4 = np.squeeze(self.k_delta * (phi_Lr_bar @ self.phi_betas + phi_L1_bar * (u_RL + u_bar)))
        c_31 = a_11 + np.squeeze(psi_m_1)
        c_41 = a_12 + np.squeeze(phi_m_1)
        d_3 = np.array([a_21 + np.squeeze(psi_m_r.T @ self.psi_betas) + (a_11 + np.squeeze(psi_m_1)) * (u_RL + u_bar)])
        d_4 = np.array([a_22 + np.squeeze(phi_m_r.T @ self.phi_betas) + (a_12 + np.squeeze(phi_m_1)) * (u_RL + u_bar)])
